...
| ID | Statut | Pri | Sujet | Note |
|---|---|---|---|---|
| AI-RACE | 🟡 | P2 | Adversaire de course + rubber-banding | Le blocage à (109, 2, -559) avait cinq causes empilées, toutes corrigées : `BulletCast<Quaternion>` décalait les composantes (tout cap relu de Bullet était faux) ; la voiture naissait dans une `DONUTFENCE` faute de `m1_snake_carstart` ; le waypoint n'était validé qu'au rayon de 8 m et elle passait à 14 m ; aucune récupération (hors-tracé / chute / coincée) ; aucun contrôle de vitesse en virage. 150 s : 0 sortie du monde. **Ne boucle toujours pas un tour** — se coince vers (26.5, 4.0, -222) sur un aller-retour du circuit, ce qui renvoie à AI-PATH |
| AI-PATH | 🟡 | P2 | Contrôleur path-following réutilisable | extrait dans `src/AI/PathFollower.*` et réutilisé par `RaceOpponent` ; `PathGraph::FindRoute` ajoute un A\*. `TrafficManager` garde ses copies. Les 966 `RoadSegment` (layout vérifié à l'octet) donnent la forme réelle des routes, et `bridgeComponents` referme les îlots : connexité **110 composantes / 4 %** → **15 / 91 %** → **1 / 100 %**, pas moyen du circuit 51 m → 15 m. Reste **105 liens de plus de 25 m** (voies multiples par `Road`, raccourcis via les nœuds `Path`) |
